...
/// <summary>
/// Shared RabbitMQ RPC client: exclusive reply queue, correlation id, envelope decoding.
/// </summary>
public sealed class RabbitRpcClient : IRabbitRpcClient, IAsyncDisposable
{
    private readonly RabbitMQSettings _settings;
    private readonly ILogger _logger;
    private readonly RabbitRpcClientOptions _options;
    ...
    public RabbitRpcClient(
        ...
        ILogger logger,
        RabbitRpcClientOptions? options = null)
    {
        _settings = opts.Value;
        _logger = logger;
        _options = options ?? new RabbitRpcClientOptions();
    ...
    public async Task<string> CallAsync(string queue, string payload, CancellationToken ct = default)
    {
        ...
        if (_options.RequireActiveConsumer)
            await EnsureActiveConsumerAsync(channel, queue);
    ...
    public async Task<bool> HasActiveConsumerAsync(string queue, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(queue))
            return false;
    ...
    private async Task EnsureActiveConsumerAsync(IChannel channel, string queue)
    {
        try
        {
